...
4. **Stars newsletter (`/stars`)**:
 - **Purpose**: Personal daily OSS newsletter (Japanese). Index at `/stars/`; editions at `/stars/<YYYY-MM-DD>/`. Generated by `tools/stars-digest/`.
 - **Search indexing**: Intentionally excluded from public search. Keep `noindex, nofollow` via `Head.astro` + nginx `X-Robots-Tag`, `Disallow` in `robots.txt`, sitemap filter in `astro.config.mjs`, and `pagefind: false` in edition/index frontmatter (also emitted by stars-digest).
 - **Feeds**: Dedicated RSS at `/stars/rss.xml` (not in tech/life RSS).
...
10. **Testing**: run `npm run lint && npm run test && npm run typecheck && npm run check-images` in `app/` before committing.
11. **Observability**: structured logging via `console.log(JSON.stringify(...))` for now; Cloud Run log analysis dashboards will be defined once telemetry stack lands.
