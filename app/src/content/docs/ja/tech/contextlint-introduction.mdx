...
> **Updated 2026-04-29**: contextlint v0.9 で LSP サーバーと VS Code 拡張によるエディタ統合が入りました。詳細は [続編記事](/ja/tech/contextlint-lsp-and-editor-integration/) を参照してください。
...
CTX-002 は、Layer 1 の用語集で定義した用語が各ドキュメントで正しく使われているかを検証します。先ほどの「ユビキタス言語の継承」の課題を、リンクの存在確認（REF-001）だけでなく、**用語レベルでの一貫性**まで踏み込んでチェックできます。
...
先ほど説明した「要件 → 仕様 → 設計 → コード」のトレーサビリティ。GRP-001 はこの**チェーンが途切れていないか**をグラフ解析で検証します。GRP-002 は A → B → C → A のような循環依存を検出し、GRP-003 はどこからも参照されていない「孤児」ドキュメントを見つけ出します。
...
v0.6 以降、contextlint には公式の Composite Action が同梱されており、`--format json` での実行と PR 差分への inline annotations までを 1 ステップでセットアップできます。
...
ドキュメントを変更した PR で自動的に検証が走り、壊れた構造を**マージ前に検出**できます。違反は PR の差分上に行レベルのアノテーションとして表示されます。

サードパーティのアクションに依存したくない場合は、素の `npx @contextlint/cli` でも構いません。`--format json` と組み合わせれば、自前のレポーターに食わせるのも簡単です。
...
4. `impact-analysis` で変更の影響範囲を確認し、関連ドキュメントも更新
...
v0.7.0 で追加された Context Compiler は、プロジェクトのドキュメントと contextlint の設定から Claude Code 向けのカスタムスキル（`SKILL.md`）を**決定論的に**生成する機能です。LLM は使わず、同じ入力なら常に同じ出力になります。
...
注目したいのは `Document Dependencies` セクションの `` !`npx contextlint impact $ARGUMENTS` `` という記法です。これは Claude Code の[**動的コンテキスト注入**（Dynamic Context Injection）](https://code.claude.com/docs/en/skills#inject-dynamic-context)を利用しています。スキルが読み込まれるときにコマンドが自動実行され、その結果がプロンプトに埋め込まれます。

つまり Claude Code がドキュメント関連の作業を始めるとき、以下が自動的に行われます。

1. ドキュメント構造とルールを SKILL.md から把握する
2. `contextlint impact` / `slice` の結果から変更の影響範囲を理解する
3. その情報をもとにコードを生成・修正する
...
<RichLinkCard
  href="/ja/tech/contextlint-lsp-and-editor-integration/"
  title="contextlint v0.9 — エディタで波線が出るようになった話（LSP / VSCode 対応）"
  description="続編記事：LSP サーバーと VS Code 拡張によるエディタ内リアルタイム診断"
/>
