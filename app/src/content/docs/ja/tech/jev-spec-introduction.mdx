...
仕様書の中に `REQ-AUTH-02`（認証に関する要件 ID）のような記述が存在し、書式が正しく、ドキュメント間で正しく参照されている。ここまでは静的解析（contextlint など）で確認できます。しかし「`src/auth/session.ts` は今も `REQ-AUTH-02` のとおりに動くのか」は、文書の構文解析だけでは分かりません。
...
「この高速な決定プリミティブがあれば、毎回の git commit でコードが仕様や、そこに関連する要件を満たしているか判定できるのではないか」
