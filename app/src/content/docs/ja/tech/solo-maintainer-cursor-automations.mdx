...
ソロメンテナーが、検証、出荷、上流への追従といった回り続ける保守ループを、エージェントと CI にどう分担させるか。その設計を実際の構成に沿って書きます。
...
| Automation | 指示ファイル | ペアの workflow | 受け持ち |
| --- | --- | --- | --- |
...
UI に置くプロンプトは、手順ファイルを指す 1 行だけです。

<figure class="theme-figure">
  <SiteImage src={waveShipperSettingsLight} alt="wave-shipper の設定画面。トリガーは毎日 09:00 GMT+9 で、Agent Instructions は手順ファイルを指す 1 行だけ" variant="inline" class="theme-light" />
  <SiteImage src={waveShipperSettingsDark} alt="wave-shipper の設定画面。トリガーは毎日 09:00 GMT+9 で、Agent Instructions は手順ファイルを指す 1 行だけ" variant="inline" class="theme-dark" />
</figure>
...
チャットと違ってリポジトリはバージョン管理されているので、指示の変更が diff としてレビューでき、エージェント製品を乗り換えても指示が生き残ります。
...
Large 層を無制限に回すと、ソロメンテナーは請求と後片付けで潰れます。
...
ユーザーが import する `terradart_google` のファクトリは、メンテナーが `terradart wrap` という生成コマンドで作ってコミットしています。
入力は 2 系統です。
...
Automations（判断と起草）と GitHub Actions（実行とゲート）を分けると、ソロメンテナーでも OSS の保守ループを継続的に回せます。
TerraDart 固有の事情を除くと、核は 3 つです。

- エージェントに merge させない
- 実クラウドに触る検証は台帳で決め、分類していないものは動かさない
- 判断はチャットではなくリポジトリに置いて、バージョン管理する

この構成は TerraDart で現役で動いていて、指示ファイルもゲートもすべてリポジトリで読めます。
