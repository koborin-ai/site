---
title: ソロメンテナーのための Cursor Automations 設計パターン
description: TerraDart で実践 — cloud agents が PR を開き、GitHub Actions が検証する週次ループ
draft: true
# ogImage: /og/solo-maintainer-cursor-automations.png  # 公開前に専用 OG を生成
...
<SiteImage src={terradartOg} alt="ソロメンテナーのための Cursor Automations 設計パターン" variant="articleHeader" style="width: 100%; height: auto;" />
...
そうして組み直したのが、**Cursor Automations** と **GitHub Actions** に役割を分担させた週次ループです。
...
ソロメンテナが OSS の外側ループ（コードそのものではなく、検証、出荷、上流への追従といった回り続ける維持作業）をエージェントと CI にどう分担させるか、その設計を実際の構成に沿って書きます。
...
このループを安心して回すには、PR ごとに何をどこまで検証するかを決める必要があります。
...
もともと GCP の見積もり用に作っていた自作の MCP サーバー gcp-cost-mcp-server（Cloud Billing Catalog API から SKU 単価を引けます）を、このループに組み込んであります。
...
外側ループの中で cloud agents が何を作っているのか、Terraform バインディングに興味がある方向けに内側だけ圧縮して置きます。
...
TerraDart 固有の事情を除くと、残るのは次の 5 点です。

- [ ] Automations のプロンプトは正本を指す 1 行にして、手順はリポジトリ側に書いたか
- [ ] エージェントに merge させず、別の workflow が変更範囲と CI（必要なら apply 実績）を確認してから merge する形にしたか
- [ ] 実クラウドに触る検証は台帳で管理し、分類していないリソースは apply しない側に倒したか
- [ ] 課金が読めない、組織専用など、人間に返す条件をエージェントの手順書に書いたか
- [ ] ルールを文章で書いて終わりにせず、lint かスクリプトで機械的に閉じたか

## まとめ

Automations（判断と起草）と GitHub Actions（実行とゲート）を分けると、ソロメンテナでも週次で OSS を回す外側ループが組めます。
核は 3 つです。
...
続きが気になる方には、[`AGENTS.md`](https://github.com/nozomi-koborinai/terradart/blob/main/AGENTS.md) と Cursor の [Automations ドキュメント](https://cursor.com/docs/cloud-agent/automations.md)がそのまま実物です。
