...
<SiteImage src={diagnosticImage} alt="A TBL-001 violation underlined in the editor with rule details on hover" variant="inline" />
...
<SiteImage src={overviewImage} alt="Multiple rule violations highlighted simultaneously in a single Markdown file" variant="inline" />
...
<SiteImage src={quickfixPopupImage} alt="Quick Fix popup offering to fill empty cells with TODO for TBL-002" variant="inline" />
*The Quick Fix for TBL-002 — a single Code Action fills the empty cell with a `TODO` placeholder.*

<SiteImage src={quickfixAppliedImage} alt="Editor state after applying the TBL-002 and CHK-001 Quick Fixes" variant="inline" />
