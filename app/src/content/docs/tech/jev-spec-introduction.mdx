...
3. **Evaluate rubrics** — submits focused evaluation questions (Rubrics) to Jev, which evaluates the spec and code as context in a single pass and returns probabilities in parallel
...
- **`specPath`** — path to the target Markdown specification file
- **`codePaths`** — glob patterns for code files implementing those requirements (with negation support to exclude test files)
- **`rubrics`** — requirement checks written as natural-language questions using `noul(...)` (keyed by requirement IDs)
- **`assertions`** — minimum confidence threshold for passing (such as `minProbability: 0.85`)
...
- **Staged checks in pre-commit hooks** — using tools like husky or simple-git-hooks, run `npx jev-spec check --staged` on every commit. Because it evaluates only modified targets in milliseconds, it catches drift before code is committed without interrupting your flow
- **Full checks in CI** — in your CI pipeline (such as GitHub Actions), run a full `npx jev-spec check` across all targets to guarantee spec compliance before merging pull requests
...
- **English is officially recommended** — while official documentation recommends English and notes that [CJK languages may not reach the same accuracy](https://docs.typesafe.ai/models#language-support), in practice Jev also understands Japanese specifications and Rubrics quite well and can evaluate them effectively
