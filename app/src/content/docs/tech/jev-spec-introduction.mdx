...
According to the official description, a System One model understands natural language as an LLM does, but it does not generate text. It returns typed answers and probabilities. A question of the kind called Noul, one that can be answered with yes or no, comes back as the probability of yes, a number between 0 and 1. The model writes no explanation of its reasoning and no code.

That property suits a build gate. Ask a general-purpose LLM "does this code satisfy the requirement?" and you get prose back: you have to parse it, its shape changes from run to run, and you pay for every generated token. Jev's answer is a number from the start, so all that is left is a comparison with a threshold. It is priced on input tokens only, and output is free ([prices](https://docs.typesafe.ai/models)).

To be clear, I am not going to argue that "Jev is not an LLM, so the concerns of the previous article do not apply". Jev, too, is a model that reads natural language and answers with probabilities, and nothing guarantees that an answer is right. TypeSafe itself writes that calibration is measured across groups of predictions and does not guarantee that an individual answer is correct. That is why I had to measure for myself.
...
3. **Asks** — puts one focused question per requirement to Jev, which reads the spec and the code once and answers every question with a probability
4. **Compares** — compares each probability with a threshold, and exits with code 1 if any assertion is violated

The configuration is TypeScript. One part of a spec paired with the code that implements it is called a **target**.
...
A question is called a **rubric**, and the comparison with a threshold an **assertion**. A run prints the probability and the verdict for every requirement (the layout is what the CLI prints; the probabilities are illustrative).
...
A full run on itself, 8 targets and 22 questions, took under four seconds. By the estimate that jev-spec prints in its report, one such run costs about $0.0006. One target took 0.3 to 0.9 seconds. The CLI itself starts in about 0.1 seconds on Node, so the request is where the time goes.

What worried me in the previous article was that tokens grow with the documents. jev-spec sends one request per target, and what it sends is the excerpt of the spec for that target and one to three files. It never sends the whole repository. Running it on every commit did not add up to an amount I would notice.
...
The development of 0.3.0 was the first time I checked jev-spec systematically against the real API, using its own specifications. The first result was a disappointment.
...
The pre-commit hook that the README recommended was unusable in that form. 0.3.0 changes the design: the diff only chooses which targets to check, and each chosen target is sent in full.
...
- **A commit that changes only a spec selects nothing in a diff run** — only code changes choose targets, so keep a full run in CI
